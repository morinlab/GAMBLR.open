...
get_cnv_and_ssm_status = function(genes_and_cn_threshs,
  ...
  if(check_cnv){
    ...
    if(!missing(cn_matrix)){
      ...
      if(missing(seg_data)){
        seg_data = get_cn_segments(these = these_samples_metadata,projection=genome_build)
        if(nrow(seg_data)==0){
            stop("no segments found for these samples. Try another genome_build or sample set?")
        }
      }
      cn_matrix = GAMBLR.utils::segmented_data_to_cn_matrix(
        seg_data = seg_data,
        regions=regions,
      ...
      if(nrow(these_samples_metadata)==0){
        stop("no samples in the metadata have copy number data")
      }
  ...
  # get maf data
  if(missing(maf_df)){
    my_maf = GAMBLR.open::get_all_coding_ssm(
      these_samples_metadata = these_samples_metadata,
      projection = genome_build,
