GAMBLR.core <- c("GAMBLR.data", "GAMBLR.helpers", "GAMBLR.utils", "GAMBLR.viz", "GAMBLR.predict")
...
GAMBLR_attach_message <- function(to_load) {
    ...
    header <- cli::rule(
        left = cli::style_bold("Welcome GAMBLer! Attaching open GAMBLR packages"),
        right = paste0("GAMBLR.open ", package_version_h("GAMBLR.open"))
    )
...
.onAttach <- function(...) {
    if (!isTRUE(getOption("GAMBLR.open.shown_startup_msg"))) {
        ...
        options(GAMBLR.open.shown_startup_msg = TRUE)
    }
}
